...
/*sqldb 데이터베이스에서 다음조건을 처리하세요.
  ○ 사용자별로구매이력을출력함
  ○ 모든컬럼을출력함
  ○ 구매이력이없는정보는출력하지않음

  앞의결과에서 userID가 'JYP'인 데이터만 출력하세요.*/
...
select u.userID, u.name, procName, u.addr, concat(u.mobile1, u.mobile2)
from usertbl u
...
where addr='서울'
UNION ALL
select name, addr, usertbl.mobile1
from usertbl
where mobile1 ='011';
